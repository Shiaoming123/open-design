...
describe('od references CLI',()=>{
  ...
  it('keeps search/show/recommend on shared endpoints and supports prompt files',async()=>{
    ...
    const detail={id:'poster:one',captureDepth:'artifact-study',useCases:['launch'],visualTraits:['grid'],sourceUrlHashes:['0123456789abcdef'],files:{source:'poster/one.json'}};
    const server=http.createServer((req,res)=>{let raw='';req.on('data',c=>raw+=c);req.on('end',()=>{seen.push({method:req.method||'',url:req.url||'',body:raw?JSON.parse(raw):null});res.setHeader('content-type','application/json');res.end(JSON.stringify(req.method==='GET'?{reference:detail}:{results:[{id:'poster:one'}],total:1}));});});
    ...
    try{
      for(const args of [['search','swiss poster'],['search','--prompt-file',prompt]]){const result=await run(base,args);expect(result.code,result.stderr).toBe(0);expect(()=>JSON.parse(result.stdout)).not.toThrow();}
      const shown=await run(base,['show','poster:one']);expect(shown.code,shown.stderr).toBe(0);expect(JSON.parse(shown.stdout).reference).toEqual(detail);
      const recommended=await run(base,['recommend','--prompt-file',prompt]);expect(recommended.code,recommended.stderr).toBe(0);
